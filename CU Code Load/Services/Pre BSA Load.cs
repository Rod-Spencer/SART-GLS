...
namespace Segway.Modules.SART.CodeLoad
{
    ...
    {
        /// <summary>Public Method - PreBSALoad</summary>
        /// <param name="side">CAN_CU_Sides</param>
        /// <param name="obj">SART_Event_Object</param>
        /// <returns>Boolean</returns>
        public Boolean PreBSALoad(CAN_CU_Sides side, out SART_Event_Object obj)
        {
            try
            {
                logger.Debug("Entered - {0}", MethodBase.GetCurrentMethod().Name);
                logger.Debug("Creating instance of SART_Event_Object");
                obj = new SART_Event_Object(InfrastructureModule.Current_Work_Order.Work_Order_ID,
                                            Event_Types.Load_BSAA_Code + (side - CAN_CU_Sides.A),
