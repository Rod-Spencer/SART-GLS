...
namespace Segway.RST
{
    ...
    public partial class Shell : Window, IActiveAware
    {
        private static NLog.Logger logger = Logger_Helper.GetCurrentLogger();

        private IUnityContainer _container;
        private IEventAggregator _EventAgrrevator;
        ...
        public Shell(IUnityContainer container, IEventAggregator aggregator)
        {
            ...
            try
            {
                ...
                _EventAgrrevator.GetEvent<Shell_Minimize_Event>().Subscribe(Minimize, true);
                _EventAgrrevator.GetEvent<Shell_Maximize_Event>().Subscribe(Maximize, true);
                _EventAgrrevator.GetEvent<Shell_Restore_Event>().Subscribe(Restore, true);
                _EventAgrrevator.GetEvent<Shell_Close_Event>().Subscribe(Close, true);
                _EventAgrrevator.GetEvent<Shell_Drag_Event>().Subscribe(Drag, true);
                _EventAgrrevator.GetEvent<Shell_MouseCursor_Busy_Event>().Subscribe(BusyMouseCursorEventHandler, ThreadOption.UIThread);
                ...
                String appWindowLoc = App_Settings_Helper.GetConfigurationValue("ApplicationWindowStartLoc", "CenterScreen");
                ...
                String appState = App_Settings_Helper.GetConfigurationValue("ApplicationState", "Normal");

                //if (appState == "Normal")
                //{
                String appWidth = App_Settings_Helper.GetConfigurationValue("ApplicationWidth", "1500");
                Double width = 0.0;
                if (Double.TryParse(appWidth, out width) == true)
                {
                    mainWindow.Width = width;
                }
                String appHeight = App_Settings_Helper.GetConfigurationValue("ApplicationHeight", "850");
                Double height = 0.0;
                ...
                String appTop = App_Settings_Helper.GetConfigurationValue("ApplicationTop", "-1");
                Double top = 0.0;
                ...
                String appLeft = App_Settings_Helper.GetConfigurationValue("ApplicationLeft", "-1");
                Double left = 0.0;
        ...
        protected override void OnClosed(EventArgs e)
        {
            try
            {
                ...
                _EventAgrrevator.GetEvent<Shell_Close_Event>().Publish("Closing Work Order");
                base.OnClosed(e);
            }
            catch (Exception ex)
            {
        ...
        private void WindowBase_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            else if ((IsRightControlKeyPressed == true) && (IsRightShiftKeyPressed == true))
            {
                ...
                else if (e.Key == Key.T)
                {
                    ...
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Trace");
                }
                else if (e.Key == Key.D)
                {
                    Logger_Helper.ChangeLoggingLevel(LogLevel.Debug);
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Debug");
                }
                else if (e.Key == Key.I)
                {
                    Logger_Helper.ChangeLoggingLevel(LogLevel.Info);
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Info");
                }
                else if (e.Key == Key.W)
                {
                    Logger_Helper.ChangeLoggingLevel(LogLevel.Warn);
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Warn");
                }
                else if (e.Key == Key.E)
                {
                    Logger_Helper.ChangeLoggingLevel(LogLevel.Error);
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Error");
                }
                else if (e.Key == Key.F)
                {
                    Logger_Helper.ChangeLoggingLevel(LogLevel.Fatal);
                    _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish("Logging level has been set to Fatal");
                }
            }
        }
        ...
        {
            IsWindowActive = true;
            _EventAgrrevator.GetEvent<StatusBar_ParentWindow_Active_Event>().Publish(true);
            _EventAgrrevator.GetEvent<Application_Activated_Event>().Publish(true);
        }

        private void mainWindow_Deactivated(object sender, EventArgs e)
        {
            IsWindowActive = false;
            _EventAgrrevator.GetEvent<StatusBar_ParentWindow_Active_Event>().Publish(false);
            _EventAgrrevator.GetEvent<Application_Activated_Event>().Publish(false);
        }
        ...
        {
            _EventAgrrevator.GetEvent<StatusBar_Region1_Color_Event>().Publish(Brushes.Snow);
            _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish(msg);
        }


        private void DisplayError(String msg)
        {
            _EventAgrrevator.GetEvent<StatusBar_Region1_Color_Event>().Publish(Brushes.Pink);
            _EventAgrrevator.GetEvent<StatusBar_Region1_Event>().Publish(msg);
        }
        ...
        private void OnClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                Configuration_Helper.Set_WebConfig_AppSetting("ApplicationWidth", mainWindow.Width.ToString());
                Configuration_Helper.Set_WebConfig_AppSetting("ApplicationHeight", mainWindow.Height.ToString());
            }
            Configuration_Helper.Set_WebConfig_AppSetting("ApplicationState", mainWindow.WindowState.ToString());
            Configuration_Helper.Set_WebConfig_AppSetting("ApplicationTop", mainWindow.Top.ToString());
            Configuration_Helper.Set_WebConfig_AppSetting("ApplicationLeft", mainWindow.Left.ToString());
        }
