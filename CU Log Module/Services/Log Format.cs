...
namespace Segway.Modules.CU_Log_Module
{
    ...
    {
        public static Boolean Format_CU_Log(Login_Context user, out String cuSerial, CAN_CU_Sides side = CAN_CU_Sides.A)
        {
            try
            {
                ...
                if (user == null)
                {
                    InfrastructureModule.Aggregator.GetEvent<StatusBar_Region1_Event>().Publish("User's Login Context has not been set");
                    return false;
                }

                Boolean pass = true;
                logger.Debug("Registering object instances: Work_Order, aggregator");
                //CU_Log_Module.CU_Log_Module_Container.RegisterInstance<Work_Order>("Work_Order", workorder, new ContainerControlledLifetimeManager());
                //CU_Log_Module.CU_Log_Module_Container.RegisterInstance<IEventAggregator>("aggregator", aggregator, new ContainerControlledLifetimeManager());
                ...
                {
                    /////////////////////////////////////////////////////////////////
                    logger.Debug("Formatting CU Log");
                    pass = CAN2_Commands.Format_CU_Log(can, InfrastructureModule.Current_Work_Order.PT_Serial, out cuSerial, side, SART_Common.Start_Process, SART_Common.End_Process, Validate_Process);
                    /////////////////////////////////////////////////////////////////
                }
                else
                {
                ...
                //logevent.EventType = (WorkOrder_Events)((int)WorkOrder_Events.Formated_CUA_Log + (int)side - (int)CAN_CU_Sides.A);
                ...
                //if (logevent != null)
                var sts = SART_Common.Create_Event(WorkOrder_Events.Formated_CUA_Log + (side - CAN_CU_Sides.A), pass ? Event_Statuses.Passed : Event_Statuses.Failed, obj.ID);
                if (sts == true)
                {
